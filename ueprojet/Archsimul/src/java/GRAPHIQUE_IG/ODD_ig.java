...
import Logique.gates.ODDParity;

public class ODD_ig extends AbstractComponent {
    private final int width = 35;
    private final int height = 42;
    private final int offsetY = 5;
    
    public ODD_ig() {
        super("ODD PARITY", 5, 1);
        setOpaque(false);
       
        /*création de la porte Logique */
        ODDParity oddparityLogique= new ODDParity("ODDParity"+System.currentTimeMillis(),5, getX(), getY());
        setComposant(oddparityLogique);
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                for (java.awt.Point port : getInputPorts()) {
                    if (port.distance(clickPoint) < 10) {
                        System.out.println("Port d'entrée ODD PARITY sélectionné !");
                    }
                }
                for (java.awt.Point port : getOutputPorts()) {
                    if (port.distance(clickPoint) < 10) {
                        System.out.println("Port de sortie ODD PARITY sélectionné !");
                    }
                }
            }
        });
    ...
    protected void paintComponent(Graphics g) {
        ...
        int text1Y = y + (h - text1Height) / 2 - text1Height / 2;  
