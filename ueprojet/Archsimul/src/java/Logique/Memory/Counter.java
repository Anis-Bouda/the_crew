...
public class Counter extends Composant{
        ...
        if(this.inputs.size()==n+2 && this.outputs.size()==n)
        {
            ...
            if(clock==State.TRUE)
            {
                ...
                if(up_count==State.TRUE)
                {
                    ...
                    addGate.setInputs(2*n+1,State.FALSE);
                    addGate.evaluate();
                    for(int i=0;i<n;i++)
                    {
    ...
    @Override
            ...
            Counter cont = (Counter) obj;

            if (this.n != cont.n) {
                return false;
            }
            if (this.up_count != cont.up_count) {
                return false;
            }
            if (this.cmp.size() != cont.cmp.size()) {
                return false;
            }
            for (int i = 0; i < this.cmp.size(); i++) {
                if (this.cmp.get(i) != cont.cmp.get(i)) {
                    return false;
                }
            }

            if (!this.addGate.equals(cont.addGate)) {
                return false;
            }
            if (!this.subGate.equals(cont.subGate)) {
                return false;
            }
            ...
            public int hashCode() {
                ...
                result = 71 * result + (this.up_count != null ? this.up_count.hashCode() : 0);
                
                for (State state : this.cmp) {
                    result = 31 * result + (state != null ? state.hashCode() : 0);
                }
            
                result = 31 * result + (this.addGate != null ? this.addGate.hashCode() : 0);
                result = 31 * result + (this.subGate != null ? this.subGate.hashCode() : 0);
            ...
            public String toString() {
                return super.toString() + ", n=" + this.n + ", up_count=" + this.up_count + ", cmp=" + this.cmp + 
                        ", addGate=" + this.addGate.toString() + ", subGate=" + this.subGate.toString();
            }
}
