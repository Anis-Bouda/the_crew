...
public abstract class AbstractComponent3 extends AbstractComponent { 
    ...
    protected void updatePortPositions() {
        ...
        for (int i = 0; i < numInputs; i++) {
            ...
            int yPos = i * spacing+20;  // Positionner les ports avec un espacement vertical
            inputPorts.add(new java.awt.Point(xPos, yPos));
        }

        // Positionner les ports de sortie (output)
        for (int i = 0; i < numOutputs; i++) {
            int xPos = 0;  // Tous les ports de sortie sont à gauche (x = 0)
            int yPos = height - (numOutputs - i) * spacing;  // Positionner les ports de sortie en bas à gauche
            outputPorts.add(new java.awt.Point(xPos, yPos));
        }
    }
