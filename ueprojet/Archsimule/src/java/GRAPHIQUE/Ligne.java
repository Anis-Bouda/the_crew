...
public class Ligne extends JPanel {
    ...
    private Point start;
    private Point end;
    private Color color = Color.GRAY; // couleur par défaut
    ...
    /** Update uniquement les extrémités du segment existant */
    public void updateEndpoints(Point start, Point end) {
        if (!segments.isEmpty()) {
            LineSegment seg = segments.get(0);
            seg.start = alignToGrid(start);
            seg.end   = alignToGrid(end);
            repaint();
        }
    }

    /** Update l’état logique (donc la couleur) du segment */
    public void updateConnectionState(State newState) {
        if (!segments.isEmpty()) {
            segments.get(0).state = newState;
            repaint();
        }
    }
