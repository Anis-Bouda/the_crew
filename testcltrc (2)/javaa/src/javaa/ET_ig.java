...
public class ET_ig extends AbstractComponent {
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                for (java.awt.Point port : getOutputPorts()) {
                    if (port.distance(clickPoint) < 10) {
                        System.out.println("ort de sortie AND sélectionné !");
                    }
                }
            }
        });
    ...
    private java.util.List<Principale.State> inputStates = java.util.Arrays.asList(Principale.State.UNKNOWN, Principale.State.UNKNOWN);
    private Principale.State outputState = Principale.State.UNKNOWN;

    // mettre a jour l'etat 
    public void setInputState(int index, Principale.State s) {
        inputStates.set(index, s);
    }

    // recuperation de l'etat 
    public Principale.State getSelectedInput(int index) {
        return inputStates.get(index);
    }

    // mettre a jour la sortie dans la sortie graphique 
    public void updateOutput(Principale.State output) {
