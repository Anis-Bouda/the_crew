...
public class LED_ig extends AbstractComponent {
    ...
    public void updateFromModel(State modelState) {
        // Ce State vient du modèle logique (True, False, UNKNOWN)
        if (modelState == State.True) {
            this.evaluate(State.NORMAL); // Allume la LED (rouge)
        } else if (modelState == State.False) {
            this.evaluate(State.OFF); // Éteint la LED (gris)
        } else {
            this.evaluate(State.INDETERMINATE); // État inconnu
        }
    }
   ...
   // initialisation par defaut 
    private State inputStates = State.UNKNOWN;
