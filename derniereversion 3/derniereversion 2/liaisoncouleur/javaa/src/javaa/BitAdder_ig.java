...
public class BitAdder_ig extends AbstractComponent {
    ...
    public BitAdder_ig() {
        super("BIT ADDER", 2, 2); // 2 entrées (A, B) et 2 sorties (S, Carry)
        setOpaque(false);

         /*la création de la porte logique */ 
        BiteADDER BitAlogique= new BiteADDER(TOOL_TIP_TEXT_KEY, ABORT, offsetY);
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Sorties (S et Carry)
        g2.drawLine(x + w, y + (int)(0.4 * h), x + w + 20, y + (int)(0.4 * h)); // S
        g2.drawLine(x + w, y + (int)(0.8 * h), x + w + 20, y + (int)(0.8 * h)); // Carry
    }
}
