...
public class MainLayout extends JPanel {
 ...
 // Taille du tableau, ajustée selon tes besoins
                    ...
                    default:
                       // int index = extractIndexFromFileName(componentName);
                        ...
                        if (componentName.startsWith("saved_circuit_")) {
                            ...
                            int numInputs = ReadInputCount.getInputCount(componentName + ".xml");  // 🔧 à implémenter
                            newComponent = new SavedX_ig(numInputs);
                        } else {
                        	   // 1) Récupère le nombre d’entrées enregistré dans le XML
                            ...
                            // 3) Crée la vue graphique
                            MyGate_ig view = new MyGate_ig(2, xmlPath);
                            // 4) Crée le composant logique à attacher
  ...
  // réutilise votre méthode existante
    ...
    private void saveCustomComponent() {
        ...
        // 4) Sélection d’une icône… (inchangé)
