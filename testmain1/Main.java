public class Main {
    public static void main(String[] args) {
        try {
            ...
            Composant ram = new RAM("RAM1", 15, 0);
            Composant rom = new ROM("ROM1", 16, 0);
            Composant power = new Power("Power1", 17, 0);
            Composant ground = new Ground("Ground1", 18, 0);
            Composant buffer = new Buffer("Buffer1", 19, 0);
            Composant controlledBuffer = new ControlledBuffer("ControlledBuffer1", 20, 0);
            Composant controlledInverter = new ControlledInverter("ControlledInverter1", 21, 0);
            PullResistor pullResistorTrue = new PullResistor("PullResistorTrue", 0, 0, State.TRUE);
            ...
            TransmissionGate transmissionGate = new TransmissionGate("TransmissionGate1", 4, 0);
            ...
            circuit.addComposant(buffer);
            circuit.addComposant(controlledBuffer);
            circuit.addComposant(controlledInverter);
            ...
            ram.setInputs(0, State.TRUE);  // Adresse
            ram.setInputs(1, State.FALSE); // Donnée à écrire
            ram.setInputs(2, State.TRUE);  // Signal d'écriture

            rom.setInputs(0, State.TRUE);  // Adresse

            buffer.setInputs(0, State.TRUE);  // Signal à transmettre

            controlledBuffer.setInputs(0, State.TRUE);  // Signal à transmettre
            controlledBuffer.setInputs(1, State.TRUE);  // Signal de contrôle

            controlledInverter.setInputs(0, State.TRUE);  // Signal à inverser
            controlledInverter.setInputs(1, State.TRUE);  // Signal de contrôle

            System.out.println("Test du PullResistor avec pullState TRUE...");
            pullResistorTrue.setInputs(0, State.UNKNOWN);
            pullResistorTrue.evaluate();
            assert pullResistorTrue.getOutput(0) == State.TRUE : "Erreur dans le PullResistor TRUE";
            System.out.println("Test réussi : PullResistor TRUE");

            System.out.println("Test du PullResistor avec pullState FALSE...");
            pullResistorFalse.setInputs(0, State.UNKNOWN);
            pullResistorFalse.evaluate();
            assert pullResistorFalse.getOutput(0) == State.FALSE : "Erreur dans le PullResistor FALSE";
            System.out.println("Test réussi : PullResistor FALSE");

            System.out.println("Test du Probe...");
            probe.setInputs(0, State.TRUE);
            probe.evaluate();
            assert probe.getOutput(0) == State.TRUE : "Erreur dans le Probe avec signal TRUE";

            probe.setInputs(0, State.FALSE);
            probe.evaluate();
            assert probe.getOutput(0) == State.FALSE : "Erreur dans le Probe avec signal FALSE";
            System.out.println("Test réussi : Probe");

            // Test du Tunnel
            System.out.println("Test du Tunnel...");
            tunnel.setInputs(0, State.TRUE);
            tunnel.evaluate();
            assert tunnel.getOutput(0) == State.TRUE : "Erreur dans le Tunnel avec signal TRUE";
            System.out.println("Test réussi : Tunnel");

            // Test du Transistor
            System.out.println("Test du Transistor...");
            transistor.setInputs(0, State.TRUE);  // signal d'entrée
            transistor.setInputs(1, State.TRUE);  // signal de contrôle (on)
            transistor.evaluate();
            assert transistor.getOutput(0) == State.TRUE : "Erreur dans le Transistor avec signal de contrôle TRUE";

            transistor.setInputs(1, State.FALSE);  // signal de contrôle (off)
            transistor.evaluate();
            assert transistor.getOutput(0) == State.FALSE : "Erreur dans le Transistor avec signal de contrôle FALSE";
            System.out.println("Test réussi : Transistor");

            // Test du TransmissionGate
            System.out.println("Test du TransmissionGate...");
            transmissionGate.setInputs(0, State.TRUE);  // signal d'entrée
            transmissionGate.setInputs(1, State.TRUE);  // signal de contrôle (on)
            transmissionGate.evaluate();
            assert transmissionGate.getOutput(0) == State.TRUE : "Erreur dans le TransmissionGate avec signal de contrôle TRUE";

            transmissionGate.setInputs(1, State.FALSE);  // signal de contrôle (off)
            transmissionGate.evaluate();
            assert transmissionGate.getOutput(0) == State.UNKNOWN : "Erreur dans le TransmissionGate avec signal de contrôle FALSE";
            System.out.println("Test réussi : TransmissionGate");

            // Simuler le circuit et afficher l'état final
            ...
            System.out.println("RAM : " + ram.getOutput(0));
            System.out.println("ROM : " + rom.getOutput(0));
            System.out.println("Power : " + power.getOutput(0));
            System.out.println("Ground : " + ground.getOutput(0));
            System.out.println("Buffer : " + buffer.getOutput(0));
            System.out.println("ControlledBuffer : " + controlledBuffer.getOutput(0));
            System.out.println("ControlledInverter : " + controlledInverter.getOutput(0));
