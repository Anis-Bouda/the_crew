...
public abstract class AbstractComponent extends JPanel {
    ...
    /*public List<State> getInputStates() 
    {
    	...
    	for (Port port : inputPorts) {
        	states.add(StateMapper.toLogicState(port.getState()));
    	}
    	return states;
    }*/
	...
	public List<State> getInputStates() 
	{
    		...
    		for (int i = 0; i < inputPorts.size(); i++) 
    		{
        		State inputState = State.UNKNOWN;

        	/*  Parcourir les connexions pour trouver celle liée à ce port 
            * lorsque onlis les 2 port on cree un conexion et chaque port a un type soit input soit output 
            *on rentre que dans les connections "les files" lié au composant concerné
            *
            *
            * 
            */
    	...
    	public void setInputsStates(List<State> states) 
    	{
    	...
    	public void updateOutputState() 
    	{
    		...
    		if (logic != null) 
    		{
        		List<State> outputs = logic.getOutputs();
